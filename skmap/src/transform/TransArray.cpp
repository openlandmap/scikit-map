...
void applyPermutation(Eigen::Ref<MatFloat> sorted_matrix,
                      Eigen::Ref<MatUint> perm_matrix,
                      bool sort_each_row_true_sort_each_col_false) {
...
void TransArray::copyVecInMatrixRow(Eigen::Ref<VecFloat> in_vec,
                                    uint_t row_idx) {
  skmapAssertIfTrue(
      (uint_t)m_data.cols() != (uint_t)in_vec.size(),
  ...
  auto copyRowsChunk = [&](uint_t i) {
    ...
    m_data.row(row_idx).segment(col_start, col_end - col_start) =
        in_vec.segment(col_start, col_end - col_start);
  };
...
void TransArray::reorderArray(Eigen::Ref<MatFloat> out_data,
  ...
  skmapAssertIfTrue((indices_matrix.size() != (uint_t)out_data.rows()) ||
                        (indices_matrix[0].size() * (uint_t)m_data.cols() !=
                         (uint_t)out_data.cols()),
                    "scikit-map ERROR 5: size of the new array does not match "
                    "the input array and the required reordering");
  auto reorderArrayRow = [&](uint_t i) {
    for (uint_t j = 0; j < indices_matrix[i].size(); j++) {
      out_data.block(i, j * m_data.cols(), 1, m_data.cols()) =
          m_data.row(indices_matrix[i][j]);
    }
  };
...
void TransArray::inverseReorderArray(
    ...
    std::vector<std::vector<uint_t>> indices_matrix) {
  skmapAssertIfTrue((indices_matrix.size() != (uint_t)out_data.rows()),
                    "scikit-map ERROR 6: size of the new array does not match "
                    "the size of the reordering matrix");
  auto inverseReorderArrayCol = [&](uint_t i) {
    out_data.row(i) = m_data.block(indices_matrix[i][0],
                                   indices_matrix[i][1] * out_data.cols(), 1,
                                   out_data.cols());
  };
...
void TransArray::slidingWindowClassMode(Eigen::Ref<MatFloat> out_data,
  ...
  auto slidingWindowClassModeCol = [&](uint_t j) {
    ...
    for (uint_t i = 0; i < (uint_t)m_data.rows(); ++i) {
      ...
      uint_t end =
          (uint_t)std::min((int)m_data.rows(), (int)i + (int)half_window + 1);
      std::vector<uint_t> window(col_data.data() + start,
                                 col_data.data() + end);
      ...
      if (dominant_classes.size() == 1) { // Single dominant class
        ...
        out_data(i, j) = float_t(*std::min_element(dominant_classes.begin(),
                                                   dominant_classes.end()));
      }
    }
  };
...
void TransArray::blocksAverage(Eigen::Ref<MatFloat> in1,
                               ...
                               uint_t y) {
  auto blocksAverageChunk = [&](Eigen::Ref<MatFloat> chunk, uint_t row_start,
                                uint_t row_end) {
    chunk.array() =
        0.25 *
        (in1.block(row_start, y * n_pix, row_end - row_start, (y + 1) * n_pix)
             .array() +
         in1.block(row_start, (y + 1) * n_pix, row_end - row_start,
                   (y + 2) * n_pix)
             .array() +
         in2.block(row_start, y * n_pix, row_end - row_start, (y + 1) * n_pix)
             .array() +
         in2.block(row_start, (y + 1) * n_pix, row_end - row_start,
                   (y + 2) * n_pix)
             .array());
  };
...
void TransArray::blocksAverageVecs(Eigen::Ref<MatFloat> in1,
                                   Eigen::Ref<MatFloat> in2, uint_t n_pix,
                                   uint_t y, uint_t row_offset) {
  auto blocksAverageVecsElem = [&](uint_t i) {
    m_data(row_offset, i) =
        0.25 * (in1(0, i + y * n_pix) + in1(0, i + (y + 1) * n_pix) +
...
void TransArray::swapRowsValues(std::vector<uint_t> row_select,
                                float_t value_to_mask, float_t new_value) {
  auto swapRowValues = [&](uint_t i) {
    m_data.row(row_select[i]) =
        (m_data.row(row_select[i]).array() == value_to_mask)
            .select(new_value, m_data.row(row_select[i]));
  };
  this->parForRange(swapRowValues, row_select.size());
}

void TransArray::maskNan(std::vector<uint_t> row_select, float_t new_value) {
  auto swapRowValues = [&](uint_t i) {
...
void TransArray::maskNanRows(std::vector<uint_t> row_select,
  ...
  skmapAssertIfTrue((row_select.size() != (uint_t)new_value_vec.size()),
                    "scikit-map ERROR 48: row_select and new_value_vec must be "
                    "of the same size");
  auto swapRowValues = [&](uint_t i) {
    auto tmp_row = m_data.row(row_select[i]);
...
void TransArray::maskDataRows(std::vector<uint_t> row_select,
  ...
  auto maskDataRow = [&](uint_t i) {
    m_data.row(row_select[i]) =
        (mask.row(0).array() == value_of_mask_to_mask)
            .select(new_value_in_data, m_data.row(row_select[i]));
  };
  this->parForRange(maskDataRow, row_select.size());
}
...
void TransArray::texturesBwTransform(Eigen::Ref<MatFloat> texture_2, float_t k,
                                     float_t a, Eigen::Ref<MatFloat> sand,
                                     Eigen::Ref<MatFloat> silt,
                                     Eigen::Ref<MatFloat> clay) {
  auto texturesBwTransformChunk = [&](Eigen::Ref<MatFloat> chunk,
                                      uint_t row_start, uint_t row_end) {
    // Extract the relevant block from texture_1 and texture_2
    MatFloat x1 = Eigen::pow(2., chunk.array());
    MatFloat x2 = Eigen::pow(
        2., texture_2.block(row_start, 0, row_end - row_start, texture_2.cols())
                .array());

    // Compute the inverse transformation
    MatFloat C = ((1. - (x1.array() + x2.array() - 2.) * k).array() /
                  (x1.array() + x2.array() + 1.).array())
                     .cwiseMax(0.);
    ...
    MatFloat scale_factor =
        (total.array() > 0)
            .select(a / total.array(),
                    0.); // If total > 0, scale; else, set to 0

    // Store results in the respective blocks
    sand.block(row_start, 0, row_end - row_start, sand.cols()) =
        S.array() * scale_factor.array();
    silt.block(row_start, 0, row_end - row_start, silt.cols()) =
        L.array() * scale_factor.array();
    clay.block(row_start, 0, row_end - row_start, clay.cols()) =
        C.array() * scale_factor.array();

    // Apply NaN mask where necessary (assuming NaN handling is required)
    MatBool mask =
        (chunk.array().isNaN() ||
         texture_2.block(row_start, 0, row_end - row_start, texture_2.cols())
             .array()
             .isNaN());

    sand.block(row_start, 0, row_end - row_start, sand.cols()) = mask.select(
        nan_v,
        sand.block(row_start, 0, row_end - row_start, sand.cols()).array());
    clay.block(row_start, 0, row_end - row_start, clay.cols()) = mask.select(
        nan_v,
        clay.block(row_start, 0, row_end - row_start, clay.cols()).array());
    silt.block(row_start, 0, row_end - row_start, silt.cols()) = mask.select(
        nan_v,
        silt.block(row_start, 0, row_end - row_start, silt.cols()).array());
  };
...
void TransArray::fitPercentage(Eigen::Ref<MatFloat> in1,
                               Eigen::Ref<MatFloat> in2) {
  auto fitPercentageChunk = [&](Eigen::Ref<MatFloat> chunk, uint_t row_start,
                                uint_t row_end) {
    chunk.array() =
        100. /
        (in1.block(row_start, 0, row_end - row_start, in1.cols()).array() +
...
void TransArray::computeNormalizedDifference(
  ...
  skmapAssertIfTrue((positive_indices.size() != negative_indices.size()) ||
                        (positive_indices.size() != result_indices.size()),
                    "scikit-map ERROR 3: positive_i, negative_i, result_i must "
                    "be of the same size");
  for (size_t i = 0; i < result_indices.size(); i++) {
    ...
    auto computeNormalizedDifferenceElement = [&](uint_t j) {
      float_t val = (m_data(positive_i, j) * positive_scaling -
                     m_data(negative_i, j) * negative_scaling) /
                        (m_data(positive_i, j) * positive_scaling +
                         m_data(negative_i, j) * negative_scaling) *
                        result_scaling +
                    result_offset;
      val = std::round(val);
    ...
    this->parForRange(computeNormalizedDifferenceElement,
                      (uint_t)m_data.cols());
  }
}
...
void TransArray::computeNirv(std::vector<uint_t> nir_indices,
                             ...
                             float_t nir_scaling, float_t red_scaling,
                             float_t result_scaling, float_t result_offset,
                             std::vector<float_t> clip_value) {
  skmapAssertIfTrue(
      (nir_indices.size() != red_indices.size()) ||
          (nir_indices.size() != result_indices.size()),
  ...
  for (size_t i = 0; i < result_indices.size(); i++) {
    ...
    auto computeNirvElement = [&](uint_t j) {
      float_t val =
          ((((m_data(nir_i, j) * nir_scaling - m_data(red_i, j) * red_scaling) /
             (m_data(nir_i, j) * nir_scaling +
              m_data(red_i, j) * red_scaling)) -
            0.08) *
           (m_data(nir_i, j) * nir_scaling)) *
              result_scaling +
          result_offset;
...
void TransArray::computeBsi(std::vector<uint_t> swir1_indices,
                            ...
                            std::vector<uint_t> result_indices,
                            float_t swir1_scaling, float_t red_scaling,
  ...
  auto computeBsiRow = [&](uint_t i, Eigen::Ref<MatFloat::RowXpr> row) {
    ...
    row =
        (m_data.row(swir1_i) * swir1_scaling + m_data.row(red_i) * red_scaling -
         m_data.row(nir_i) * nir_scaling - m_data.row(blue_i) * blue_scaling)
                .array() /
            (m_data.row(swir1_i) * swir1_scaling +
             m_data.row(red_i) * red_scaling + m_data.row(nir_i) * nir_scaling +
             m_data.row(blue_i) * blue_scaling)
                .array() *
            result_scaling +
        result_offset;
    row = (row.array()).round();
...
void TransArray::computeEvi(std::vector<uint_t> red_indices,
  ...
  auto computeEviRow = [&](uint_t i, Eigen::Ref<MatFloat::RowXpr> row) {
    ...
    uint_t nir_i = nir_indices[i];
    uint_t blue_i = blue_indices[i];
    row = ((m_data.row(nir_i) * nir_scaling - m_data.row(red_i) * red_scaling)
               .array() /
           ((m_data.row(nir_i) * nir_scaling +
             m_data.row(red_i) * red_scaling * 6.0 -
             m_data.row(blue_i) * blue_scaling * 7.5)
                .array() +
            1.0)
               .array())
                  .array() *
...
void TransArray::computeFapar(std::vector<uint_t> red_indices,
  ...
  auto computeFaparRow = [&](uint_t i, Eigen::Ref<MatFloat::RowXpr> row) {
    ...
    uint_t nir_i = nir_indices[i];
    row =
        ((((((m_data.row(nir_i) * nir_scaling - m_data.row(red_i) * red_scaling)
                 .array() /
             (m_data.row(nir_i) * nir_scaling + m_data.row(red_i) * red_scaling)
                 .array()) -
            0.03)
...
void TransArray::computeGeometricTemperature(
  ...
  auto computeGeometricTemperatureRow = [&](uint_t i,
    ...
    row = latitude
              .unaryExpr([&day_of_year, &a, &b](float_t lat) {
                float_t cos_teta =
                    std::cos(((day_of_year - 18.) / 182.5 +
                              std::pow(4., (float_t)(lat < 0.))) *
                             M_PI);
              ...
              .array() -
          0.006 * elevation_scaling * elevation.array();
...
void TransArray::extractIndicators(Eigen::Ref<MatFloat> data_out,
  ...
  skmapAssertIfTrue(col_out_select.size() != classes.size(),
                    "scikit-map ERROR 38: size of col_out_select differs from "
                    "size of col_in_select times size of classes");

  auto extractIndicatorsChunk = [&](Eigen::Ref<MatFloat> chunk,
                                    uint_t row_start, uint_t row_end) {
    for (uint_t k = 0; k < classes.size(); ++k) {
      data_out.block(row_start, col_out_select[k], row_end - row_start, 1) =
          (chunk.col(col_in_select).array() == classes[k]).cast<float_t>();
    }
  };
...
void TransArray::computePercentiles(std::vector<uint_t> col_in_select,
  ...
  auto computePercentilesChunk = [&](Eigen::Ref<MatFloat> rows_block,
    ...
    for (uint_t i = 0; i < (uint_t)sorted_chunk.rows(); ++i) {
      std::vector<float_t> sorted_row(sorted_chunk.row(i).data(),
                                      sorted_chunk.row(i).data() +
                                          sorted_chunk.row(i).size());
    ...
    for (uint_t k = 0; k < percentiles.size(); ++k) {
      ...
      for (uint_t i = 0; i < (uint_t)sorted_chunk.rows(); ++i) {
        if (not_nan_count(i) == 0) {
          ...
          float_t percentile_pos =
              (float_t)(not_nan_count(i) - 1) * (percentiles[k] / 100.0);
          ...
          if (percentile_pos_floor == percentile_pos_ceil) {
            out_data(row_start + i, col_out_select[k]) =
                sorted_chunk(i, percentile_pos_floor);
          } else {
            float_t percentile_val_floor =
                sorted_chunk(i, percentile_pos_floor) *
                ((float_t)percentile_pos_ceil - percentile_pos);
            float_t percentile_val_ceil =
                sorted_chunk(i, percentile_pos_ceil) *
                (percentile_pos - (float_t)percentile_pos_floor);
            out_data(row_start + i, col_out_select[k]) =
                percentile_val_floor + percentile_val_ceil;
          }
        }
      }
    }
...
void TransArray::fitProbabilities(Eigen::Ref<MatFloat> out_data,
                                  float_t input_scaling, uint_t target_scaling,
                                  Eigen::Ref<MatFloat> best_classes_data,
                                  uint_t n_best_classes) {
  skmapAssertIfTrue(out_data.cols() != m_data.cols(),
                    "scikit-map ERROR 56: in and out data should have the same "
                    "number of columns");
  skmapAssertIfTrue((uint_t)best_classes_data.cols() != n_best_classes,
                    "scikit-map ERROR 57: best_classes_data must have "
                    "n_best_classes columns");
  ...
  auto fitProbabilitiesChunk = [&](Eigen::Ref<MatFloat> in_chunk,
                                   uint_t row_start, uint_t row_end) {
    ...
    best_classes_data.block(row_start, 0, n_pix, n_best_classes) =
        perm_matrix.block(0, n_classes - n_best_classes, n_pix, n_best_classes)
            .cast<float_t>();
...
#pragma omp parallel for
    for (uint_t i = 0; i < n_pix; ++i) {
      ...
      while (tmp_sum != target_scaling) {
        ...
        mix_vec.tail(n_classes - mid) =
            out_chunk_ceil.row(i).tail(n_classes - mid);
        ...
        if (tmp_sum == target_scaling || power_of_two < 0)
          ...
          skmapAssertIfTrue(
              mid < 0,
              "Mid should not get inferior to 0, something went wrong");
        } else {
...
void TransArray::maskDifference(float_t diff_th, uint_t count_th,
  ...
  auto maskDifferenceChunk = [&](Eigen::Ref<MatFloat> chunk, uint_t row_start,
                                 uint_t row_end) {
    auto ref_data_chunk =
        ref_data.block(row_start, 0, row_end - row_start, ref_data.cols());
    auto mask_out_chunk =
        mask_out.block(row_start, 0, row_end - row_start, ref_data.cols());
    ...
    // If the masking is recurring in several columns it is not considered
    // outlier and so not masked
...
void TransArray::applyTsirf(Eigen::Ref<MatFloat> out_data,
                            uint_t out_index_offset, float_t w_0,
                            Eigen::Ref<VecFloat> w_p, Eigen::Ref<VecFloat> w_f,
                            bool keep_original_values,
                            const std::string &version,
                            const std::string &backend) {
  auto applyTsirfChunk = [&](Eigen::Ref<MatFloat> chunk, uint_t row_start,
                             uint_t row_end) {
...
void TransArray::convolveRows(Eigen::Ref<MatFloat> out_data, float_t w_0,
...
void TransArray::averageAggregate(Eigen::Ref<MatFloat> out_data,
                                  uint_t agg_factor) {
  auto averageAggregateChunk = [&](Eigen::Ref<MatFloat> chunk, uint_t row_start,
                                   uint_t row_end) {
    uint_t n_chucks = std::ceil((float_t)chunk.cols() / (float_t)agg_factor);
    skmapAssertIfTrue((uint_t)out_data.cols() != n_chucks,
                      "scikit-map ERROR 34: wrong number of columns of the "
                      "aggregated matrix");
    for (uint_t i = 0; i < n_chucks; ++i) {
      uint_t col_start = i * agg_factor;
      uint_t col_end = std::min((i + 1) * agg_factor, (uint_t)chunk.cols());
      out_data.col(i).segment(row_start, row_end - row_start) =
          chunk.middleCols(col_start, col_end - col_start).rowwise().mean();
    }
  };
  this->parChunk(averageAggregateChunk);
}

void TransArray::nanMeanAggregatePattern(
    Eigen::Ref<MatFloat> out_data,
    std::vector<std::vector<uint_t>> &agg_pattern) {
  auto nanMeanAggregatePatternChunk = [&](Eigen::Ref<MatFloat> chunk,
                                          uint_t row_start, uint_t row_end) {
    skmapAssertIfTrue(
        (uint_t)out_data.cols() != agg_pattern.size(),
    ...
    for (uint_t i = 0; i < agg_pattern.size(); ++i) {
      ...
      VecFloat count =
          (float_t)chunk_col_sel.cols() -
          chunk_col_sel.array().isNaN().cast<float_t>().rowwise().sum().array();
...
void TransArray::nanMean(Eigen::Ref<VecFloat> out_data) {
  auto nanMeanChunk = [&](Eigen::Ref<MatFloat> chunk, uint_t row_start,
                          uint_t row_end) {
    auto res = out_data.segment(row_start, row_end - row_start);
    VecFloat count =
        (float_t)chunk.cols() -
        chunk.array().isNaN().cast<float_t>().rowwise().sum().array();
  ...
  auto computeMannKendallPValuesChunk = [&](Eigen::Ref<MatFloat> chunk,
    ...
    for (uint_t i = 0; i < (uint_t)chunk.rows(); i++) {
      ...
      if (s > 0.)
        ...
        z_score = (s + 1.) / std::sqrt(var_s);
      // p-value
      out_data(row_start + i) =
          2. * (1. - standardNormalCdf(std::abs(z_score)));
    }
  };
...
void TransArray::linearRegression(Eigen::Ref<VecFloat> x,
  ...
  skmapAssertIfTrue((uint_t)m_data.cols() != (uint_t)x.size(),
                    "scikit-map ERROR 37: regression variable is assumed to "
                    "match size of data columns");
  auto linearRegressionChunk = [&](Eigen::Ref<MatFloat> chunk, uint_t row_start,
                                   uint_t row_end) {
    ...
    beta_1.segment(row_start, row_end - row_start) =
        covariance.array() / x_variance;
