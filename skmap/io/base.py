...
class RasterData(SKMapBase):
    ...
    def predict(self, model, X, predict_proba: bool = False) -> np.ndarray:
        """sklearn-style prediction on a 2-D covariate array.

        :param model: a fitted model exposing ``predict`` (and optionally
          ``predict_proba``).
        :param X: ``(n_samples, n_features)`` covariate array.
        :param predict_proba: use ``model.predict_proba`` instead of
          ``model.predict`` (default).
        """
        fn = model.predict_proba if predict_proba else model.predict
        return fn(X)
    ...
    def predict_raster(
        ...
        """Predict on this RasterData for **all years at once**.
        ...
        (band ``k`` → output ``k // n_years``, year ``k % n_years``) — so it
        can be written directly as one raster per (output, year).
        ...
        :param feature_names: covariate names in the model's feature order.
          ...
          Temporal covariates are matched by name **and** year (year-agnostic
          names such as ``ndvi_winter`` repeat per year); ``common`` (static)
          covariates are reused for every year.
        :param predict_proba: use ``model.predict_proba`` instead of
          ``model.predict`` (default).
        ...
        covs_idx, years = self._get_covs_idx_by_year(feature_names)
        arr = self.array.get()
        n_pixels = arr.shape[1]
        n_years = len(years)

        # Build the concatenated feature matrix (n_years·n_pixels, n_features),
    ...
    def predict_raster_to_file(
        ...
        **save_kwargs,
        ...
        major, year minor — matching the layout returned by
        :meth:`predict_raster`::

            years = self.get_years() or [None]
            out_files = [
