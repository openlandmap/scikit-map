...
class GLADLandsat:
    ...
    def _save_percentile_agg(
        self, result, base_raster, tile, start, p, output_dir, unit8
    ):
        ...
        for i in range(0, result.shape[0]):
            ...
            for paux in p:
                p_dir = output_dir.joinpath(f"{start}").joinpath(
                    f"B{i + 1}_P{int(paux)}"
                )
        ...
        output_files += save_rasters(
            fn_base_raster=base_raster,
            data=data,
            fn_raster_list=fn_raster_list,
            dtype=dtype,
            nodata=0,
            fit_in_dtype=True,
            n_jobs=self.parallel_download,
    ...
    def _calc_save_count(self, data, max_val, base_raster, tile, start, output_dir):
        ...
        band_count = bc.nansum(
            np.logical_not(np.isnan(band_data)).astype("uint8"), axis=2
        )
    ...
    def percentile_agg(
        ...
        data, url_list, base_raster = self.read(
            tile, start, end, clear_sky, min_clear_sky
        )
        ...
        if len(url_list) > 0:
            ...
            if len(result) > 0:
                ...
                if output_dir is not None:
                    ...
                    output_files = self._save_percentile_agg(
            ...
            raise RuntimeError(
                "All the images were read, but no clear_sky data was found."
            )
...
try:
    ...
    class STACGenerator:
        ...
        def __init__(
            ...
            self.additional_url_cols = list(
                coll_columns[pd.Series(coll_columns).str.startswith("url")]
            )
            ...
            self.fields_lookup = dict.fromkeys(
                set(chain(*[self.fields[key] for key in self.fields.keys()])), True
            )
        ...
        def _fetch_collection(self, key, i, row, bbox_footprint_results):
            ...
            for dt1, dt2 in date_range(
                row["start_date"].strftime(dt_fmt),
                row["end_date"].strftime(dt_fmt),
                date_unit=date_unit,
                date_step=date_step,
                ...
                for dp in depth:
                    asset_urls.append(
                        _eval(row["main_url"], {"dt": f"{dt1}_{dt2}", "dp": dp})
                    )
                    for ac_url in self.additional_url_cols:
                        if row[ac_url]:
                            asset_urls.append(
                                _eval(row[ac_url], {"dt": f"{dt1}_{dt2}", "dp": dp})
                            )
                ...
                if main_url in bbox_footprint_results:
                    ...
                    items.append(
                        self._new_item(
                            row, dt1, dt2, bbox, footprint, asset_urls, style_urls
                        )
                    )
                else:
        ...
        def _populate_vector(self) -> None:
            ...
            for rid, row in self.gsheet.collections.iterrows():
                start_date, end_date = (
                    datetime.strftime(row["start_date"], "%Y%m%d"),
                    datetime.strftime(row["end_date"], "%Y%m%d"),
                )

                start_date_item = datetime.strptime(start_date, "%Y%m%d").strftime(
                    "%Y-%m-%d"
                )
                end_date_item = datetime.strptime(end_date, "%Y%m%d").strftime(
                    "%Y-%m-%d"
                )
                ...
                item = pystac.Item(
                    ...
                    properties={
                        "start_datetime": start_date_item,
                        "end_datetime": end_date_item,
                    },
                    stac_extensions=[
                        "https://stac-extensions.github.io/file/v2.0.0/schema.json"
                    ],
                ...
                for aurl in self.additional_url_cols:
                    if row[aurl] != "":
                        aid = (
                            f"{row['id']}_{Path(row[aurl]).stem.split('?')[0].lower()}"
                        )
                        item.add_asset(
                            aid, pystac.Asset(href=row[aurl], roles=["data"])
                        )
        ...
        def _populate(self) -> None:
            ...
            for key in groups.groups.keys():
                for i, row in groups.get_group(key).iterrows():
                    ...
                    ttprint(
                        f"Generating dates for {row['id']} ({row['start_date']} => {row['end_date']})"
                    )
                    for dt1, dt2 in date_range(
                        row["start_date"].strftime(dt_fmt),
                        row["end_date"].strftime(dt_fmt),
                        ...
                        main_url = _eval(
                            row["main_url"], {"dt": f"{dt1}_{dt2}", "dp": dp}
                        )
                        args.append((main_url,))

            bbox_footprint_results = {}
            for url, bbox, footprint in parallel.job(
                self._bbox_and_footprint,
                args,
                n_jobs=-1,
            ...
            for key, row, items in parallel.job(
                ...
                if collection is None:
                    ...
                    self._verbose(
                        f"Creating collection {collection.id} with {len(items)}"
                    )
                            ...
                            if self._is_data(asset):
                                ...
                                if asset.media_type in (pystac.MediaType.COG):
                                    if "main" in asset.extra_fields:
                                        ...
                                        args.append(
                                            (asset.href, str(thumb_fn), item.id, False)
                                        )

                self._verbose(
                    f"Generating {len(args)} thumbnails for catalog {catalog.id}"
                )

                for thumb_fn, item_id, is_thumb_url in parallel.job(
                    self._thumbnail, args, n_jobs=-1
                ):
                    ...
                    if thumb_fn is not None:
                        thumd_id = (
                            self._asset_id(
                                thumb_fn, self.asset_id_delim, self.asset_id_fields
                            )
                            + "_preview"
                        )
                        roles = []
                        ...
                        if thumb_base_url is not None:
                            thumb_fn = str(thumb_fn).replace(
                                str(output_dir), thumb_base_url
                            )
        ...
        def _thumbnail(
            ...
            with rasterio.open(url) as src:
                try:
                    ...
                    result = src.read(
                        1, out_shape=(1, src.height // oview, src.width // oview)
                    ).astype("float32")
        ...
        def _new_collection(self, row, items):
            if len(items) > 0:
                ...
                row[self.THUMB_CMAP], row[self.THUMB_VMIN], row[self.THUMB_VMAX] = (
                    self.sld2cmap(row["sld_url"])
                )
            ...
            if asset.href:
                ...
                if r.status_code == 200:
                    ...
                    checksum = hashlib.md5(
                        json.dumps(headers, sort_keys=True, ensure_ascii=True).encode(
                            "utf-8"
                        )
                    ).hexdigest()
        ...
        def _new_item(
            ...
            start_date_item = datetime.strptime(
                start_date, self.url_date_format
            ).strftime("%Y-%m-%d")
            end_date_item = datetime.strptime(end_date, self.url_date_format).strftime(
                "%Y-%m-%d"
            )
            ...
            item.add_asset(
                self._asset_id(main_url, self.asset_id_delim, self.asset_id_fields),
                pystac.Asset(
                    href=main_url,
                    media_type=pystac.MediaType.COG,
                    roles=["data"],
                    extra_fields={"main": True},
                ),
            )
            for aurl in additional_urls:
                item.add_asset(
                    self._asset_id(aurl, self.asset_id_delim, self.asset_id_fields),
                    pystac.Asset(
                        href=aurl, media_type=pystac.MediaType.COG, roles=["data"]
                    ),
                )
        ...
        def _bbox_and_footprint(self, raster_fn):
            ...
            with rasterio.Env(**self.gdal_env):
                with rasterio.open(raster_fn) as ds:
                    ...
                    bounds = ds.bounds
                    left_wgs84, bottom_wgs84 = transformer.transform(
                        bounds.bottom, bounds.left
                    )
                    right_wgs84, top_wgs84 = transformer.transform(
        ...
        def save_and_publish_all(
            ...
            self.save_all(
                output_dir, catalog_type=catalog_type, thumb_base_url=thumb_base_url
            )
            ...
            args = [
                (fpath, output_dir, client, s3_bucket_name, s3_prefix)
                for fpath in files
            ]
            self._verbose(
                f"Copying {len(files)} files to {s3_host}/{s3_bucket_name}/{s3_prefix}"
            )

            for r in parallel.job(
                self._s3_fput_object,
                args,
                n_jobs=10,
        ...
        def search(self, query, field="title"):
            ...
            with self.ix.searcher() as searcher:
                ...
                for r in searcher.search(myquery):
                    result.append({"pos": i, "id": r["id"], "title": r["title"]})
                    i += 1
