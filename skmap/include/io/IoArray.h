...
public:
   ...
   *
   * @param row          Reference to an Eigen row expression where the data
   * will be stored.
   ...
   * @param value_to_mask Optional value in the dataset to treat as "masked"
   * (e.g., nodata).
   ...
   *
   * @param file_locs     Vector of file paths or URLs of raster datasets to
   * read.
   * @param perm_vec      Vector of row indices specifying the permutation order
   *                      for writing into `m_data`.
   * @param x_off         Horizontal offset of the reading window in the
   * rasters.
   * @param y_off         Vertical offset of the reading window in the rasters.
   * @param x_size        Width of the window to read.
   * @param y_size        Height of the window to read.
   * @param read_type     GDALDataType specifying the type to read (e.g.,
   * GDT_Float32).
   * @param bands_list    List of band indices to read from each dataset.
   * @param value_to_mask Optional value to treat as masked (e.g., nodata). If
   * not provided, no masking is performed.
   ...
   *
   * @note Both `value_to_mask` and `value_to_set` are forwarded to
   * `readDataCore` for per-pixel masking and replacement.
   */
  ...
  /**
   * @ingroup io
   * @brief Write portions of the internal matrix `m_data` to multiple GeoTIFF
   * files.
   *
   ...
   *
   * @param base_files Vector of file paths to base raster files to copy
   * metadata from.
   ...
   * @param data_indices Indices of rows in `m_data` corresponding to each
   * output file.
  ...
  void writeData(std::vector<std::string> base_files, std::string base_folder,
    ...
    auto writeTiff = [&](uint_t i, Eigen::Ref<MatFloat::RowXpr> row) {
      ...
      if (inputDataset->GetGeoTransform(geotransform) != CE_None) {
        throw std::runtime_error(
            "scikit-map ERROR 10: Failed to get GeoTransform.");
      }
      auto projection = inputDataset->GetProjectionRef();
      if (projection == nullptr) {
        throw std::runtime_error(
            "scikit-map ERROR 10: Failed to get ProjectionRef.");
      }
      auto spatial_ref = inputDataset->GetSpatialRef();
      if (spatial_ref == nullptr) {
        throw std::runtime_error(
            "scikit-map ERROR 10: Failed to get SpatialRef.");
      }
      ...
      if (layer_name.size() >= suffix.size() &&
          layer_name.compare(layer_name.size() - suffix.size(), suffix.size(),
                             suffix) == 0) {
      ...
      MatType init_raster =
          MatType::Constant(1, x_size_in * y_size_in, no_data_value);
      auto out_write1 = writeBand->RasterIO(
          GF_Write, 0, 0, x_size_in, y_size_in, init_raster.data(), x_size_in,
          y_size_in, write_type, 0, 0);
      skmapAssertIfTrue(out_write1 != CE_None,
                        "scikit-map ERROR 11: issues in writing the file " +
                            layer_name);
      auto out_write2 = writeBand->RasterIO(GF_Write, x_off, y_off, x_size,
                                            y_size, casted_row.data(), x_size,
                                            y_size, write_type, 0, 0);
