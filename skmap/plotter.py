...
def plot_stac_collection(
    ...
    thumb_id: str = "thumbnail",
    ncols: int = 4,
    figsize: ArrayLike = (15, 25),
    axes_pad: Union[float, Tuple[float, float]] = (0, 0.4),
) -> None:
    """
...
def plot_rasters(
    *rasters: Union[Iterable[str], Iterable[np.ndarray], Iterable[Path]],
    ...
    for i, r in enumerate(rasters):
        ...
        if perc_clip:
            try:
                ...
                data_equalized = []
                for band in bands:
                    data_equalized.append(
                        _percent_clip(rasters[i][:, :, band], perc_min, perc_max)
                    )
    ...
    if len(rasters) == 1:
        axes = [axes]
    for i, (ax, arr, cmap, nod, _vmin, _vmax) in enumerate(
        zip(
            axes,
            rasters,
            cmaps,
