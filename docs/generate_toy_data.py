...
def _out_rasters(raster_files, basedir="data"):
    ...
    for f in [
        ...
        out_files.append(
            (
                str(f)
                .replace(dt1, dt1.replace(".", ""))
                .replace(dt2, dt2.replace(".", ""))
            )
        )
...
def gen_dataset(
    ...
    save_rasters(
        static_rasters[0],
        _out_rasters(static_rasters, static_outdir),
        static_data,
        window=window,
    )
    ...
    save_rasters(
        swir1_rasters[0],
        _out_rasters(swir1_rasters, swir1_outdir),
        swir1_data,
        window=window,
        nodata=255,
    )
    ...
    save_rasters(
        ndvi_rasters[0],
        _out_rasters(ndvi_rasters, ndvi_outdir.joinpath("filled")),
        ndvi_data,
        window=window,
        nodata=255,
    )
    ...
    save_rasters(
        ndvi_rasters[0],
        _out_rasters(ndvi_rasters, ndvi_outdir.joinpath("gappy")),
        ndvi_data,
        window=window,
        nodata=255,
    )
    ...
    samples_overlaid = samples_overlaid.drop(columns=["overlay_id"]).rename(
        columns=rename_col
    )
    samples_overlaid["target"] = samples_overlaid["target"].astype("int")
    samples_overlaid["label"] = (
        samples_overlaid["target"].astype("int").astype("str").replace(lc_label_remap)
    )
    samples_overlaid["code"] = (
        samples_overlaid["target"].astype("int").astype("str").replace(lc_code_remap)
    )

    ttprint("Saving land-cover samples")
    samples_outdir.mkdir(parents=True, exist_ok=True)
    samples_overlaid[["date", "label", "code", "target", "geometry"]].to_file(
        samples_outdir.joinpath("samples.gpkg")
    )
