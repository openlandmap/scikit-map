...
elif [ "$USER" != "opengeohub" ]
  then
    ## cannot move home folder when it's a shared volume, have to copy and change permissions instead
    cp -r /home/opengeohub /home/$USER
    ## RENAME the user
    usermod -l $USER -d /home/$USER opengeohub
    groupmod -n $USER opengeohub
    usermod -a -G staff $USER
...
if [ "$GROUPID" -ne 1000 ]
## Configure the primary GID (whether opengeohub or $USER) with a different GROUPID if requested.
  then
