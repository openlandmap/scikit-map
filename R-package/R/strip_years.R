...
strip_years <- function(tif.name, year.span=c(2000:2020), type=c("begin", "end")[1], begin.seasons=c("12-02", "03-21", "06-25", "09-13", "12-02"), season.name=c("winter", "spring", "summer", "fall"), timeless="timeless",format.date="%Y/%m/%d", year.type=c("01-01","12-31")){
  if(length(grep(timeless, tif.name))>0){
    if(type=="begin"){
      x = paste0(year.span[1], "-", year.type[1])
    }
    if(type=="end"){
      x = paste0(year.span[length(year.span)], "-", year.type[2])
    }
  } else {
    s = which(sapply(sapply(season.name[1:4], function(i){ grep(i, tif.name) }), function(i){length(i)>0}))
    if(length(s)==0 & type=="begin") { s <- 1 }
    if(length(s)==0 & type=="end") { s <- 4 }
    ...
    if(type=="begin"){
      if(s == 1){
        x = paste0(year.span[y], "-", year.type[1])  
      } else {
        x = paste0(year.span[y], "-",  year.type[1])
      }
    }
    if(type=="end"){
      x = paste0(year.span[y], "-",  year.type[2])
    }
  }
  return(x)
}
