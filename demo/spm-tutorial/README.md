...
in which sample fraction for different batches varies from 50% to 70%.
    ...
    Call:
     ranger::ranger(dependent.variable.name = task$target_names, data = task$data(),      case.weights = task$weights$weight, importance = "impurity",      mtry = 2L, sample.fraction = 0.595255173859186, num.trees = 319L) 
    ...
    Splitrule:                        variance 
    OOB prediction error (MSE):       10401.41 
    R squared (OOB):                  0.1818144 
...
Note: here we just show start and the ending values.
    ...
    ...
    [[1]]
      [1] 180.32435 176.01437 157.08177 173.51803 137.27863 153.34352 182.54969
    ...
...
In case of regression task,
    ...
    plot_spm(x = df.tr[,target.variable], y = pred.v[[1]], gtype = "accuracy")

    Because of the LOW number of observations a density plot is displayed.


<img src="README_files/figure-markdown_strict/unnamed-chunk-17-2.png" alt="Accuracy plot"  />
<p class="caption">
Accuracy plot
</p>
