...
- create an R function `train.spm` to train (classification and regression Spatial Prediction Model) models using Ensemble Machine Learning on spatial data in mlr3
,
- create an R function accuracy.plot.spm to visualize accuracy plots (regression) for models produced using `train.spm`,
- create an R function `predict.spm` that can used the models fitted using `train.spm` and predict values that can be exported as a map.
