...
For demonstration we are using daily rainfall dataset from Switzerland used in the Spatial Interpolation Comparison 1997. For more info see [HERE](https://rdrr.io/github/Envirometrix/landmap/man/sic1997.html).
...
df$test5 <- (df$DEM/2**4)**6 
df$test6 <- (df$CHELSA_rainfall)**-1
df$test7 <-((df$CHELSA_rainfall)*13/34)
df$noise1 <- runif(1:nrow(df))/0.54545
df$noise2 <- sqrt(runif(1:nrow(df))) 
```

where `noise1` and `noise2` are pure noise.
...
```{r ,results='hide', warning=FALSE}
tr = eumap::train_spm(df.tr, target.variable = "rainfall", folds = 5, 
                n_evals = 3, crs = "+init=epsg:4326", coordinate_names = c("X","Y"))
```
...
<<<<<<< HEAD:demo/spm-tutorial/README.Rmd
plt = plot_spm(x = df[-train_ind, "rainfall"], y = pred.v[[1]], z = valu.imp, gmode  = "root" , gtype = "correlation", gvar_imp = "TRUE")
=======
plt = plot_spm(x = df.tr[,target.variable], y = pred.v[[1]], z = valu.imp, gmode  = "root" , gtype = "accuracy", gvar_imp = "TRUE")
>>>>>>> master:demo/spm-tutorial/archive/README.Rmd
```
